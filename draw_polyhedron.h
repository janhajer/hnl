...
struct DefaultColorFunctorPolyhedron {
    ...
    static CGAL::Color run ( const Polyhedron&, typename Polyhedron::Facet_const_handle fh )
    {
// use to get the mono color
...
/// usefull for very big object where this time could be long)
    SimplePolyhedronViewerQt ( QWidget* parent, /*const Polyhedron& apoly,*/ const char* title = "Basic Polyhedron Viewer", bool anofaces = false, const ColorFunctor& fcolor = ColorFunctor() ) :
// First draw: no vertex; edges, faces; mono-color; inverse normal
        Base ( parent, title, false, true, true, true, true ),
//         polys({apoly}),
        m_nofaces ( anofaces ),
        m_fcolor ( fcolor )
    {
...
protected:
    ...
    {
        add_segment ( he->vertex()->point(), he->opposite()->vertex()->point() );
...
// We can use add_point(p, c) with c a CGAL::Color to add a colored point
    ...
    void compute_elements()
    {
        ...
        for ( auto const& poly : polys ) {
            if ( !m_nofaces ) for ( auto f = poly.facets_begin(); f != poly.facets_end(); f++ ) if ( f != boost::graph_traits<Polyhedron>::null_face() ) {
                        compute_face ( f, poly );
                    }
            for ( auto e = poly.halfedges_begin(); e != poly.halfedges_end(); ++e ) if ( e < e->opposite() ) {
                    compute_edge ( e );
                }
...
void draw ( const CGAL_POLY_TYPE& apoly, const char* title = "Polyhedron Basic Viewer", bool nofill = false )
{
    int argc = 1;
