...
bool FunctionEncapsulator::integrateGauss(double& result, int iArg, double xLo,
  double xHi, std::vector<double> args, double tol) {
    ...
    // Precision in this bin not OK, subdivide.
      ...
      if (1.0 + c*std::abs(zDel) == 1.0) {
        ...
        std::cout << "\n FunctionEncapsulator::integrateGauss(): cannot "
             << "reach desired tolerance at double precision." << std::endl;
        result = 0.0 ;
        return false;
      }
      zHi = zMid;
...
bool FunctionEncapsulator::brent(double& solution, double targetValue,
  int iArg, double xLo, double xHi, std::vector<double> argsIn, double tol,
  int maxIter) {
